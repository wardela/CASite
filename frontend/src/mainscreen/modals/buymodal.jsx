...
export default function BuyModal({ isOpen, onClose, plan }) {
  ...
  const pricing = {
    "6": { Essential: 49.99, Premium: 64.99, Exclusive: 99.99 },
    "12": { Essential: 44.99, Premium: 59.99, Exclusive: 89.99 },
    "24": { Essential: 39.99, Premium: 54.99, Exclusive: 82.99 },
  };
...
const calculateSavings = (originalPrice, currentPrice) => {
    if (!originalPrice || !currentPrice) return "0.00";
...
const currentPrice = pricing[duration]?.[plan] || 0;
  const originalPrice = originalPricing[plan] || 0;
  const discount = calculateDiscount(originalPrice, currentPrice);
  const monthlyBalance = calculateSavings(originalPrice, currentPrice);
  const totalSavings = (monthlyBalance * parseInt(duration)).toFixed(2);
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/60 backdrop-blur-md p-4 animate-fadeIn">
      <div className="relative w-full max-w-5xl h-[90vh] max-h-[800px] rounded-3xl bg-white dark:bg-dark-2 shadow-2xl overflow-hidden animate-slideUp flex flex-col">
        ...
        <div className="absolute top-0 start-0 end-0 h-2 bg-gradient-to-r from-[#5fb875] via-[#52a868] to-[#4a9960]"></div>

        {/* Header - Fixed */}
        <div className="relative px-6 lg:px-8 py-5 bg-gradient-to-br from-[#5fb875]/5 to-transparent border-b border-gray-100 dark:border-dark-3 flex-shrink-0">
          <button
            onClick={onClose}
          ...
          <div className="pr-12">
            <div className="inline-flex items-center gap-2 mb-2 px-3 py-1 bg-[#5fb875]/10 backdrop-blur-sm rounded-full border border-[#5fb875]/20">
              <div className="w-2 h-2 bg-[#5fb875] rounded-full animate-pulse"></div>
              <span className="text-xs font-semibold text-[#5fb875]">{t("buyModal.badge")}</span>
            </div>
            
            <h3 className="text-xl lg:text-2xl font-bold text-dark dark:text-white mb-1">
              {t("buyModal.title.before")}{" "}
              <span className="bg-gradient-to-r from-[#5fb875] to-[#4a9960] bg-clip-text text-transparent">
          ...
          <div className="mt-4">
            ...
            <div className="h-2 bg-gray-200 dark:bg-dark-3 rounded-full overflow-hidden">
              <div
                className="h-full bg-gradient-to-r from-[#5fb875] to-[#4a9960] transition-all duration-500 ease-out"
            ...
            <form>
              ...
              {step === 1 && (
                <div className="space-y-6">
                  <div>
                    <h4 className="text-2xl font-bold text-dark dark:text-white mb-2">{t("buyModal.steps.duration.title")}</h4>
                    <p className="text-sm text-body-color dark:text-dark-6">{t("buyModal.steps.duration.subtitle")}</p>
                  </div>
                  ...
                  <div className="grid md:grid-cols-3 gap-4">
                    ...
                    <button
                      ...
                      className={`relative p-6 rounded-2xl border-2 transition-all duration-300 hover:scale-105 ${
                        duration === "6"
                          ? "border-[#5fb875] bg-gradient-to-br from-[#5fb875]/5 to-transparent shadow-lg shadow-[#5fb875]/20"
                          : "border-gray-200 dark:border-dark-3 hover:border-[#5fb875]/50"
                      }`}
                    >
                      <div className="text-center">
                        <div className="text-3xl font-bold text-dark dark:text-white mb-1">6</div>
                        <div className="text-sm text-body-color dark:text-dark-6 mb-3">{t("buyModal.steps.duration.months")}</div>
                        <div className="inline-block px-3 py-1 bg-[#5fb875]/10 text-[#5fb875] text-xs font-bold rounded-full mb-2">
                          {t("buyModal.steps.duration.save")} {discount}%
                        </div>
                        <div className="text-2xl font-bold text-[#5fb875]" dir="ltr">${pricing["6"]?.[plan] || "0.00"}</div>
                        <div className="text-xs text-body-color dark:text-dark-6">per month</div>
                      </div>
                      {duration === "6" && (
                        <div className="absolute top-4 end-4">
                          <svg className="w-6 h-6 text-[#5fb875]" fill="currentColor" viewBox="0 0 20 20">
                    ...
                    >
                      <span className="absolute -top-3 end-4 bg-gradient-to-r from-[#5fb875] to-[#4a9960] text-white text-xs font-bold px-3 py-1 rounded-full shadow-lg">{t("buyModal.steps.duration.most_popular")}</span>
                      <div className="text-center">
                        <div className="text-3xl font-bold text-dark dark:text-white mb-1">12</div>
                        <div className="text-sm text-body-color dark:text-dark-6 mb-3">{t("buyModal.steps.duration.months")}</div>
                        <div className="inline-block px-3 py-1 bg-[#5fb875]/10 text-[#5fb875] text-xs font-bold rounded-full mb-2">
                          {t("buyModal.steps.duration.save")} {calculateDiscount(originalPrice, pricing["12"][plan])}%
                        </div>
                        <div className="text-2xl font-bold text-[#5fb875]">${pricing["12"][plan]}</div>
                        <div className="text-xs text-body-color dark:text-dark-6">per month</div>
                      </div>
                      {duration === "12" && (
                        <div className="absolute top-4 end-4">
                    ...
                    >
                      <div className="text-center">
                        <div className="text-3xl font-bold text-dark dark:text-white mb-1">24</div>
                        <div className="text-sm text-body-color dark:text-dark-6 mb-3">{t("buyModal.steps.duration.months")}</div>
                        <div className="inline-block px-3 py-1 bg-[#5fb875]/10 text-[#5fb875] text-xs font-bold rounded-full mb-2">
                          {t("buyModal.steps.duration.save")} {calculateDiscount(originalPrice, pricing["24"][plan])}%
                        </div>
                        <div className="text-2xl font-bold text-[#5fb875]">${pricing["24"][plan]}</div>
                        <div className="text-xs text-body-color dark:text-dark-6">per month</div>
                      </div>
                      {duration === "24" && (
                        <div className="absolute top-4 end-4">
              ...
              {step === 4 && (
                <div className="space-y-6">
                  ...
                  {/* Summary Review */}
                  <div className="mt-8 p-6 bg-gradient-to-br from-[#5fb875]/5 to-transparent rounded-2xl border-2 border-[#5fb875]/20">
                    <h5 className="font-bold text-dark dark:text-white mb-4 flex items-center gap-2">
                      <svg className="w-5 h-5 text-[#5fb875]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div className="space-y-3 text-sm">
                      ...
                      <div className="flex justify-between">
                        ...
                        <span className="line-through text-gray-400" dir="ltr">${originalPrice || "0.00"}/mo</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-body-color dark:text-dark-6">{t("buyModal.steps.review.summary.your_price")}</span>
                       <span className="font-bold text-[#5fb875]" dir="ltr">${currentPrice || "0.00"}/mo</span>
                      </div>
                      <div className="pt-3 border-t border-gray-200 dark:border-dark-3 flex justify-between">
                        <span className="font-bold text-dark dark:text-white">{t("buyModal.steps.review.summary.total_savings")}</span>
                       <span className="font-bold text-green-600 dark:text-green-400" dir="ltr">${totalSavings || "0.00"}</span>
                      </div>
                    </div>
                  </div>
                </div>
          ...
          <div className="flex items-center justify-between gap-4">
            {step > 1 ? (
              <button
                type="button"
                onClick={handlePrev}
                className="group inline-flex items-center gap-2 px-6 py-3 rounded-full border-2 border-gray-300 dark:border-dark-3 text-dark dark:text-white font-semibold hover:border-[#5fb875] hover:text-[#5fb875] transition-all duration-300"
              >
            ...
            {step < totalSteps ? (
              <button
                type="button"
                onClick={handleNext}
                className="group ms-auto inline-flex items-center gap-2 px-8 py-3 rounded-full bg-gradient-to-r from-[#5fb875] to-[#4a9960] text-white font-bold shadow-lg hover:shadow-xl hover:shadow-[#5fb875]/30 transition-all duration-300 hover:scale-105"
              >
              ...
              <button
                ...
                className="group ms-auto inline-flex items-center gap-3 px-8 py-3 rounded-full bg-gradient-to-r from-[#5fb875] to-[#4a9960] text-white font-bold shadow-lg hover:shadow-xl hover:shadow-[#5fb875]/30 transition-all duration-300 hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <style jsx>{`
        ...
        .custom-scrollbar::-webkit-scrollbar-thumb:hover { background: #4a9960; }
      `}</style>
    </div>
  );
}
...
/* ===== Input Component ===== */
function Input({ label, name, type = "text", placeholder = "", value, onChange, icon }) {
  return (
    <div>
      <label className="block mb-2 text-sm font-semibold text-dark dark:text-white">
        {label}
      </label>
      <div className="relative">
        ...
        <input
          ...
          onChange={onChange}
          className={`w-full rounded-xl border-2 border-gray-200 dark:border-dark-3 ${icon ? 'pl-12' : 'pl-4'} pr-4 py-3 bg-white dark:bg-dark text-dark dark:text-white placeholder:text-gray-400 dark:placeholder:text-gray-500 focus:border-[#5fb875] focus:outline-none focus:ring-2 focus:ring-[#5fb875]/20 transition-all duration-300`}
        />
      </div>
    </div>
  );
  ...
  return (
    <div>
      ...
      <textarea
        ...
        placeholder={placeholder}
        onChange={onChange}
        className="w-full rounded-xl border-2 border-gray-200 dark:border-dark-3 px-4 py-3 bg-white dark:bg-dark text-dark dark:text-white placeholder:text-gray-400 dark:placeholder:text-gray-500 focus:border-[#5fb875] focus:outline-none focus:ring-2 focus:ring-[#5fb875]/20 transition-all duration-300 resize-none"
      />
    </div>
  );
}